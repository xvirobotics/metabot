...
| **Web UI** | 浏览器端对话 `http://server:9100/web/`（语音功能需通过 Caddy 配置 HTTPS）。WebSocket 实时流式输出、Markdown 渲染、工具调用展示、电话语音模式（VAD 自动检测说完）、MetaMemory 浏览器、明暗主题。React 19 + Vite SPA。 |
...
- **Web 电话语音模式** — Web UI 点击电话图标即可免手语音对话。VAD 自动检测说完，自动循环 录音→回复→录音。需要 HTTPS（Caddy 设置见 CLAUDE.md）
