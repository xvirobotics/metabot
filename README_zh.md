...
| `GET` | `/oauth/feishu/callback` | OAuth 回调（公开，无需认证） |
| `GET` | `/api/oauth/status` | 查询用户 OAuth 授权状态 |
| `POST` | `/api/oauth/revoke` | 撤销用户 OAuth 授权 |
| `POST` | `/api/feishu/document/create` | 以用户身份创建飞书文档 |
| `PUT` | `/api/feishu/document/:docId` | 更新飞书文档内容 |
| `POST` | `/api/feishu/document/:docId/append` | 追加内容到飞书文档 |
